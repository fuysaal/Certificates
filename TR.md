TR

* Cisco Networking Academy (Kurs Tamamlama Belgesi)
    - Ethical Hacker
    - Introduction ToCybersecurity

* BTK Akademi (Kurs Tamamlama Belgesi)
    - Uygulamalı Sızma Testi
    - Web Uygulama Sızma Testi
    - SIEM Temelleri
    - Kali Linux
    - Siber Güvenliğe Giriş
    - Siber Tehdit İstihbaratı ve Tehdit Avcılığı

* Udemy (Kurs Tamamlama Belgesi)
    - Etik Hacker
    - Ağ içi Saldırılar
    - Mobil Sızma Testi
    - Yetki Yükseltme
    - Python
    - Web Sızma Testi

* Microsoft Learning (Kurs Tamamlama Belgesi)
    - SOC Program Foundations Training

* EC-Council (Kurs Tamamlama Belgesi)
    - Understanding Cyber Attacks and Defenses
    - Building and Configuring Your Cybersecurity Attack Lab
    - Mastering OSINT and Dark Web Intelligence
    - Deep Web and Cybersecurity
    - Learn Burp Suite for Advanced Web Penetration Testing
    - OWASP API Top 10 crAPI
    - SOC for Blue Teaming
    - Preparing for a SOC Analyst Job
    - Designing & Deploying a Zero-Trust Architecture
    - Red Team Mastery: Advanced Offensive Security
    - Emulate Adversarial Activity with Atomic Red Team
    - Security Automation in AWS with Python
    - Effective Risk Management with ISO 31000
    - The Ultimate Guide to NIST SP 800-171 Standard for CUI Protection
    - Data Science for Cybersecurity
    - Machine Learning for Application in Digital Forensics
    - Generative AI for Cybersecurity
    - AI for Cybersecurity & Bug Bounty Hunting


EN

* Cisco Networking Academy (Validation of Course Completion)
    - Ethical Hacker
    - Introduction ToCybersecurity

* BTK Academy (Validation of Course Completion)
    - Applied Penetration Testing
    - Web Application Penetration Testing
    - SIEM Fundamentals
    - Kali Linux
    - Introduction to Cybersecurity
    - Cyber Threat Intelligence and Threat Hunting

* Udemy (Kurs Tamamlama Belgesi)
    - Ethical Hacker
    - Internal Network Attacks
    - Mobile Penetration Testing
    - Privilege Escalation
    - Python
    - Web Penetration Testing

* Microsoft Learning (Validation of Course Completion)
    - SOC Program Foundations Training

* EC-Council (Validation of Course Completion)
    - Understanding Cyber Attacks and Defenses
    - Building and Configuring Your Cybersecurity Attack Lab
    - Mastering OSINT and Dark Web Intelligence
    - Deep Web and Cybersecurity
    - Learn Burp Suite for Advanced Web Penetration Testing
    - OWASP API Top 10 crAPI
    - SOC for Blue Teaming
    - Preparing for a SOC Analyst Job
    - Designing & Deploying a Zero-Trust Architecture
    - Red Team Mastery: Advanced Offensive Security
    - Emulate Adversarial Activity with Atomic Red Team
    - Security Automation in AWS with Python
    - Effective Risk Management with ISO 31000
    - The Ultimate Guide to NIST SP 800-171 Standard for CUI Protection
    - Data Science for Cybersecurity
    - Machine Learning for Application in Digital Forensics
    - Generative AI for Cybersecurity
    - AI for Cybersecurity & Bug Bounty Hunting
